...
def check_for_leakage(df1, df2, patient_col):
    ...
    df1_patients_unique = set(df1[patient_col].values)  #train_df.PatientId.values
...
def get_weighted_loss(pos_weights, neg_weights, epsilon=1e-7):
    ...
    def weighted_loss(y_true, y_pred):
        ...
        for i in range(len(pos_weights)):
            ...
            loss +=  -1 * K.sum(pos_weights[i] * y_true[:, i] * K.log(y_pred[:, i])) + -1 * K.sum(neg_weights[i] * (1 - y_true[:, i]) *K.log(1 - y_pred[:, i]))
